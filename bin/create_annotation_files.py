    ...
    with open(study_ofile,'w') as ptr:
      for annotation in CONFIG.annotation_order[study]:
        ...
        for annotation_criteria,annotation_def in all_annotations.items():
          annotation_var = __obtain_annotation_var(var_consequence,annotation_def,annotation_criteria,ANNOTATION_FILE)
...
if __name__ == "__main__":
  ...
  ANNOTATION_FILE = list(
    compress(
      cargs.anno_summaries,
      [bool(re.search(f"5_1_vep_summaries_{VCF_NAME}.sqlite3.db",x)) for x in cargs.anno_summaries]
    )
  )
  assert(len(ANNOTATION_FILE)==1)
