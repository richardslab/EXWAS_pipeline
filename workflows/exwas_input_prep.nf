...
workflow EXWAS_INPUT_PREP_PIPELINE {
  ...
  main:
    ...
    regenie_input_res = CREATE_REGENIE_INPUT(
      anno_res.anno_db.collect().map{it.sort().join(" ")},
      anno_res.anno_log.collect().map{it.sort().join(" ")},
      annotation_inputs
    )

  emit:
    regenie_input_prep_mask = regenie_input_res.mask
    regenie_input_prep_setlist = regenie_input_res.setlist
    regenie_input_prep_annotation = regenie_input_res.annotation
